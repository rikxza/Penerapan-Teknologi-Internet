<x-app-layout>
    ...
    <div class="min-h-screen p-6 md:p-8 transition-colors duration-500">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 glass-card bento-card rounded-[2rem] p-8 relative overflow-hidden animate-fade-in">
                <div class="flex flex-col md:flex-row items-center gap-8">
                    ...
                    <div class="relative w-40 h-40 shrink-0">
                        <svg class="w-40 h-40 transform -rotate-90" viewBox="0 0 100 100">
                            ...
                            <circle cx="50" cy="50" r="40" fill="none" 
                                class="text-emerald-500 health-ring" 
                                stroke="currentColor" stroke-width="8" stroke-linecap="round"
                                style="--score: {{ $healthScore }}"/>
                        </svg>
                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                            <span class="text-4xl font-black text-emerald-700 dark:text-emerald-400">{{ $healthScore }}</span>
                    ...
                    <div class="text-center md:text-left flex-1">
                        <p class="text-xs font-black text-emerald-600 dark:text-emerald-500 uppercase tracking-widest mb-2">Your Financial Health Score</p>
                        <h2 class="text-3xl md:text-4xl font-black text-emerald-800 dark:text-white">{{ $healthScore }}/100</h2>
                        <p class="text-emerald-600/80 dark:text-emerald-400/80 mt-2 font-medium">{{ $healthStatus }}</p>
                        
                        <div class="flex flex-wrap gap-2 mt-4">
                            <span class="px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/20 text-emerald-700 dark:text-emerald-400">
                                💰 Savings: {{ $totalIncome > 0 ? round((($totalIncome - $totalExpense) / $totalIncome) * 100) : 0 }}%
                            </span>
                            <span class="px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/20 text-emerald-700 dark:text-emerald-400">
            ...
            <div class="glass-card bento-card rounded-[2rem] p-6 flex flex-col animate-fade-in delay-100">
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-10 h-10 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-2xl flex items-center justify-center text-xl shadow-lg shadow-indigo-500/30">
                        🤖
                    </div>
                    <div>
                ...
                <div class="glass-card rounded-2xl p-4 flex-1 mb-4">
                    <p class="text-sm text-emerald-700 dark:text-emerald-300 leading-relaxed">
                        ...
                        @else
                            ✨ Keuanganmu sehat! Kamu sudah menabung <span class="font-bold text-emerald-600 dark:text-emerald-400">{{ Auth::user()->formatCurrency($totalIncome - $totalExpense) }}</span> bulan ini.
                        @endif
                    </p>
                </div>
                
                <div class="flex gap-2">
                    <button @click="$dispatch('open-ai-chat')" class="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-2.5 px-4 rounded-xl text-xs transition-all hover:scale-[1.02]">
                        Chat with AI
                    </button>
                </div>
            </div>
            ...
            <div class="lg:col-span-3 grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="glass-card rounded-2xl p-5 group hover:scale-[1.02] transition-transform">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest">Balance</p>
                    ...
                    <p class="text-xl font-black text-emerald-800 dark:text-white">{{ Auth::user()->formatCurrency($netSavings) }}</p>
                </div>

                {{-- Income --}}
                <div class="glass-card bento-card rounded-2xl p-5">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Income</p>
                    ...
                    <p class="text-xl font-black text-emerald-800 dark:text-white">{{ Auth::user()->formatCurrency($totalIncome) }}</p>
                </div>

                {{-- Expense --}}
                <div class="glass-card bento-card rounded-2xl p-5">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-[10px] font-black text-rose-600 dark:text-rose-400 uppercase tracking-widest">Expense</p>
                    ...
                    <p class="text-xl font-black text-emerald-800 dark:text-white">{{ Auth::user()->formatCurrency($totalExpense) }}</p>
                </div>

                {{-- Savings --}}
                <div class="glass-card bento-card rounded-2xl p-5">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-widest">Budget</p>
                    ...
                    <p class="text-xl font-black text-emerald-800 dark:text-white">{{ Auth::user()->formatCurrency($totalBudgetAllocation) }}</p>
                </div>
            </div>
            ...
            <div class="glass-card bento-card rounded-[2rem] p-6 animate-fade-in delay-200">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-xs font-black text-emerald-600 dark:text-emerald-500 uppercase tracking-widest">Expense by Category</h3>
                    ...
                    <div class="mt-4 space-y-2">
                        @foreach($expenseByCategory->take(4) as $category => $amount)
                            <div class="flex items-center justify-between text-xs">
                                ...
                                <span class="font-bold text-emerald-800 dark:text-white">{{ Auth::user()->formatCurrency($amount) }}</span>
                            </div>
                        @endforeach
                    </div>
                @else
            ...
            <div class="lg:col-span-2 glass-card bento-card rounded-[2rem] p-6 animate-fade-in delay-300">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xs font-black text-emerald-600 dark:text-emerald-500 uppercase tracking-widest">Recent Transactions</h3>
                    <a href="{{ route('transactions.index') }}" class="text-[10px] font-black text-emerald-500 hover:text-emerald-600 uppercase tracking-tight">View All →</a>
                </div>
                
                <div class="space-y-4">
                    @forelse($transactions as $transaction)
                        <div class="flex items-center justify-between p-3 rounded-2xl hover:bg-white/50 dark:hover:bg-white/5 transition-all duration-200">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl bg-white dark:bg-slate-800 flex items-center justify-center text-xl border border-emerald-200/50 dark:border-slate-700 shadow-sm">
                                    {{ $transaction->category->icon ?? '💰' }}
                                </div>
                                <div>
                                    <p class="text-sm font-bold text-emerald-800 dark:text-white">{{ $transaction->description ?? 'No Description' }}</p>
                    ...
                    @empty
                        <p class="text-center text-emerald-600/60 dark:text-emerald-400/60 py-8 italic">Belum ada transaksi.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="glass-card rounded-[2rem] p-6">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xs font-black text-emerald-600 dark:text-emerald-500 uppercase tracking-widest">Budget Progress</h3>
                    <a href="{{ route('budgeting.index') }}" class="text-[10px] font-black text-emerald-500 hover:text-emerald-600 uppercase tracking-tight">Manage →</a>
                </div>
                    ...
                    @forelse($activeBudgets->take(4) as $budget)
                        <div>
                            <div class="flex justify-between mb-2">
                                <span class="text-sm font-bold text-emerald-700 dark:text-emerald-300">{{ $budget->category->name }}</span>
                                <span class="text-xs font-black {{ $budget->percentage > 100 ? 'text-rose-500' : 'text-emerald-600 dark:text-emerald-400' }}">
                                    {{ $budget->percentage }}%
                                </span>
                            </div>
                            ...
                            <p class="text-[10px] text-emerald-600/60 dark:text-emerald-400/60 mt-1">
                                {{ Auth::user()->formatCurrency($budget->realized) }} / {{ Auth::user()->formatCurrency($budget->amount) }}
                            </p>
                        </div>
                    @empty
                        <p class="text-center text-emerald-600/60 dark:text-emerald-400/60 py-4 italic">Belum ada budget.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="lg:col-span-3 flex flex-wrap gap-4 justify-center">
                <a href="{{ route('scan.receipt') }}" class="glass-card px-6 py-3 rounded-2xl flex items-center gap-3 hover:scale-[1.02] transition-transform group">
                ...
                <a href="{{ route('transactions.create') }}" class="glass-card px-6 py-3 rounded-2xl flex items-center gap-3 hover:scale-[1.02] transition-transform group">
