<x-app-layout>
    ...
    <div x-data="receiptScanner()" x-init="init()">
        ...
        <div class="px-8 py-6">
            ...
            <p class="text-slate-500 dark:text-slate-400 text-xs font-bold tracking-wide uppercase transition-colors duration-500">Upload struk belanja untuk input pengeluaran otomatis dengan AI</p>
        </div>
        ...
        <div class="mx-4 md:mx-8 mb-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {{-- UPLOAD AREA --}}
            <div class="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-[2rem] shadow-xl dark:shadow-2xl transition-all duration-500 overflow-hidden p-8">
                <h2 class="text-xl font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-3">
                    <div class="w-10 h-10 money-gradient rounded-xl flex items-center justify-center shadow-lg shadow-emerald-500/30">
                ...
                >
                    ...
                    <template x-if="previewUrl">
                        <div class="relative">
                            ...
                            <button @click.stop="clearImage()" class="absolute top-2 right-2 bg-red-500 text-white p-2 rounded-full shadow-lg hover:bg-red-600 transition">
            ...
            <div class="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-[2rem] shadow-xl dark:shadow-2xl transition-all duration-500 overflow-hidden p-8">
                ...
                <template x-if="!receiptData">
                    <div class="text-center py-16">
                        <div class="w-20 h-20 mx-auto mb-4 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center">
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span 
                                ...
                                x-text="receiptData.confidence"
                            ></span>
                        </div>

                        {{-- Merchant Name --}}
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Nama Toko</label>
                            <input type="text" x-model="receiptData.merchant_name" class="w-full px-4 py-3 bg-slate-100 dark:bg-slate-800 border-none rounded-xl focus:ring-2 focus:ring-emerald-500 dark:text-white">
                        </div>

                        {{-- Date --}}
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Tanggal</label>
                            <input type="date" x-model="receiptData.transaction_date" class="w-full px-4 py-3 bg-slate-100 dark:bg-slate-800 border-none rounded-xl focus:ring-2 focus:ring-emerald-500 dark:text-white">
                        </div>

                        {{-- Amount --}}
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Total</label>
                            <div class="relative">
                                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-bold">Rp</span>
                                <input type="number" x-model="receiptData.total_amount" class="w-full pl-12 pr-4 py-3 bg-slate-100 dark:bg-slate-800 border-none rounded-xl focus:ring-2 focus:ring-emerald-500 dark:text-white">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select x-model="selectedCategory" class="w-full px-4 py-3 bg-slate-100 dark:bg-slate-800 border-none rounded-xl focus:ring-2 focus:ring-emerald-500 dark:text-white">
                        ...
                        <div>
                            ...
                            <input type="text" x-model="description" :placeholder="'Belanja di ' + (receiptData.merchant_name || '')" class="w-full px-4 py-3 bg-slate-100 dark:bg-slate-800 border-none rounded-xl focus:ring-2 focus:ring-emerald-500 dark:text-white">
                        </div>

                        {{-- Items List (if available) --}}
                        <template x-if="receiptData.items && receiptData.items.length > 0">
                            <div>
                                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Items</label>
                                <div class="space-y-2 max-h-40 overflow-y-auto">
                                    <template x-for="(item, index) in receiptData.items" :key="index">
                                        <div class="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg text-sm">
                                            <span class="text-slate-700 dark:text-slate-300" x-text="item.name"></span>
                                            <span class="text-slate-500 dark:text-slate-400" x-text="'Rp ' + (item.price || 0).toLocaleString('id-ID')"></span>
                                        </div>
                                    </template>
                                </div>
                            </div>
