<x-app-layout>
    ...
    <div x-data="{ showForm: false, isNewCategory: false }">
        ...
        {{-- WRAPPER --}}
        <div class="mx-4 md:mx-8 mb-8 p-8 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-[3rem] shadow-xl">

            {{-- NOTIF --}}
            @if (session('status'))
                <div class="mb-6 p-4 bg-emerald-500/10 border border-emerald-500/50 text-emerald-500 rounded-2xl text-sm font-bold">
                    {{ session('status') }}
                </div>
            @endif
            ...
            @if (session('error_budget'))
                <div class="mb-6 p-4 bg-rose-500/10 border border-rose-500/50 text-rose-500 rounded-2xl text-sm font-bold flex items-center gap-3">
            ...
            {{-- FORM CREATE --}}
            <div x-show="showForm" x-transition class="mb-12 p-8 bg-slate-50 dark:bg-slate-900/60 rounded-[2rem] border border-slate-200 dark:border-slate-800">
                ...
                <form method="POST" action="{{ route('budgeting.store') }}"
                    ...
                    <div x-data="{ isNewCategory: false }" class="flex flex-col gap-2">
                        ...
                        <div x-show="!isNewCategory">
                            <select name="category_id" x-bind:required="!isNewCategory"
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" value="{{ old('amount') }}" required class="w-full bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl p-3 focus:ring-2 focus:ring-emerald-500">
                    </div>

                    {{-- PERIOD --}}
                    <div>
                        <label class="text-[10px] font-black text-slate-400 uppercase ml-2">Period</label>
                        <select name="period" class="w-full bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl p-3 focus:ring-2 focus:ring-emerald-500">
                    ...
                    <div class="md:col-span-3 flex gap-3">
                        <button type="submit" class="px-6 py-3 bg-emerald-500 text-white text-xs font-black uppercase rounded-xl hover:bg-emerald-600 transition-all">
                            Add Budget
                        </button>
                ...
                @forelse ($budgets as $budget)
                    <div x-data="{ editing: false }"
                        ...
                        <div class="flex justify-between mb-6">
                            <div>
                                <h3 class="text-lg font-black text-slate-900 dark:text-white">{{ $budget->category->name }}</h3>
                            ...
                            <div class="flex gap-2">
                                {{-- EDIT BUTTON --}}
                                <button @click="editing = !editing" class="p-2 text-slate-400 hover:text-blue-500 transition-colors">
                                ...
                                <form method="POST" action="{{ route('budgeting.destroy', $budget->id) }}">
                                    @csrf @method('DELETE')
                                    <button type="submit" onclick="return confirm('Hapus budget?')" class="p-2 text-slate-400 hover:text-rose-500 transition-colors">
                        ...
                        <div x-show="editing" x-transition
                            ...
                            <form method="POST" action="{{ route('budgeting.update', $budget->id) }}"
                                  class="flex gap-3">
                                @csrf
                                @method('PATCH')

                                <input type="number" name="amount"
                                       value="{{ (int)$budget->amount }}"
                                ...
                                <button type="submit" class="bg-emerald-500 text-white px-4 rounded-xl hover:bg-emerald-600 transition-all">
