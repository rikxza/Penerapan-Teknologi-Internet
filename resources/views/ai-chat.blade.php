<x-app-layout>
    ...
    <div x-data="{ showForm: false }">
        ...
        <div class="px-4 md:px-8 py-4 md:py-6">
            <h1 class="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white transition-colors duration-500">AI
                Assistant
            </h1>
            <p
                class="text-slate-500 dark:text-slate-400 text-xs font-bold tracking-wide uppercase transition-colors duration-500">
                Tanya G-ment tentang kondisi keuanganmu</p>
        </div>

        {{-- 2. WADAH UTAMA CHAT --}}
        <div
            class="mx-4 md:mx-8 mb-8 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-[2.5rem] md:rounded-[3.5rem] shadow-xl dark:shadow-2xl transition-all duration-500 overflow-hidden flex flex-col h-[calc(100vh-220px)]">

            <div
                class="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/20">
                <div class="flex items-center gap-4">
                    <div
                        class="w-10 h-10 money-gradient rounded-xl rotate-12 flex items-center justify-center shadow-xl shadow-emerald-500/40 shrink-0 transition-transform hover:rotate-0 duration-300">
                        <span
                            class="font-black text-white text-xl -rotate-12 group-hover:rotate-0 transition-transform">$</span>
                    </div>
                    <div>
                        <h3 class="text-lg font-black text-slate-900 dark:text-white leading-tight">G-ment AI Assistant
                        </h3>
                        <div class="flex items-center gap-2">
                            <div class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>
                            <span
                                class="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Online</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="chat-container"
                class="flex-1 overflow-y-auto p-6 md:p-10 space-y-8 custom-scrollbar bg-slate-50/30 dark:bg-transparent">

                {{-- Welcome Message --}}
                <div class="flex flex-col items-start gap-3 max-w-[85%] md:max-w-[70%]">
                    <div
                        class="bg-white dark:bg-slate-800/60 p-6 rounded-[2rem] rounded-tl-none shadow-sm border border-slate-100 dark:border-slate-700">
                        <p class="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
                            Halo <strong>{{ Auth::user()->name }}</strong>! Saya <strong>G-ment</strong>. Saya sudah
                            menganalisa data transaksi Anda bulan ini. Ada yang ingin Anda tanyakan atau konsultasikan?
                        </p>
                    </div>
                </div>
            ...
            <div class="p-6 md:p-8 bg-white dark:bg-slate-900/40 border-t border-slate-100 dark:border-slate-800">
                <div class="relative group max-w-4xl mx-auto">
                    ...
                    <button onclick="sendMessage()" id="send-btn"
                        ...
                        <svg id="loading-icon" class="hidden animate-spin h-5 w-5 text-white"
