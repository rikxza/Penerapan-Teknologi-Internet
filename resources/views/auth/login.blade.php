    ...
    <div class="w-full max-w-6xl glass-card rounded-[40px] overflow-hidden shadow-2xl transition-all duration-500">
        ...
        <header class="flex justify-between items-center p-8">
            ...
            <div class="flex items-center gap-6">
                ...
                <a href="{{ route('login') }}" class="text-sm font-bold text-emerald-500 dark:text-emerald-400 border-b-2 border-emerald-500 dark:border-emerald-400 pb-1">LOGIN</a>
                <a href="{{ route('register') }}" class="bg-slate-900 dark:bg-white/10 hover:bg-slate-800 dark:hover:bg-white/20 transition-all text-white px-6 py-2 rounded-full text-sm font-semibold">REGISTER</a>
            </div>
        </header>
        ...
        <div class="flex flex-col md:flex-row p-8 gap-12">
            ...
            <div class="w-full md:w-1/2 money-gradient rounded-[35px] p-12 relative overflow-hidden min-h-[450px] flex flex-col justify-end group shadow-2xl shadow-emerald-500/20">
                <div class="relative z-10">
                    <h1 class="text-5xl font-black leading-tight text-white">Welcome back,<br>Smart Earner!</h1>
            ...
            <div class="w-full md:w-1/2 flex flex-col justify-center max-w-md mx-auto">
                <div class="mb-8">
                    <h2 class="text-3xl font-bold text-slate-900 dark:text-white transition-colors">Login</h2>
                    <p class="text-slate-500 dark:text-gray-400 text-sm mt-2">Enter your credentials to access your dashboard.</p>
                </div>
                
                <div class="flex gap-3 mb-8">
                    <button class="flex-1 bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 py-2.5 rounded-xl text-[10px] font-bold tracking-wider text-orange-500 transition-all">GOOGLE ACCOUNT</button>
                </div>

                <div class="relative flex py-4 items-center">
                    <div class="flex-grow border-t border-slate-200 dark:border-white/10"></div>
                    <span class="flex-shrink mx-4 text-slate-400 dark:text-gray-500 text-[10px] uppercase tracking-widest">Or login with email</span>
                    <div class="flex-grow border-t border-slate-200 dark:border-white/10"></div>
                </div>
                ...
                <form action="{{ route('login') }}" method="POST" class="space-y-5">
                    @csrf
                    <div>
                        <input type="email" name="email" placeholder="Email Address" class="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-4 text-sm text-slate-900 dark:text-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition-all placeholder:text-slate-400" required>
                    </div>
                    <div>
                        <input type="password" name="password" placeholder="Password" class="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-4 text-sm text-slate-900 dark:text-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition-all placeholder:text-slate-400" required>
                    </div>
                    
                    <div class="flex items-center justify-between py-2">
                        <div class="flex items-center gap-2">
                            <input type="checkbox" id="remember" class="w-4 h-4 rounded bg-slate-200 dark:bg-white/10 border-slate-300 dark:border-white/10 text-emerald-500 focus:ring-emerald-500">
                            <label for="remember" class="text-xs text-slate-500 dark:text-gray-400">Remember me</label>
                        </div>
                        <a href="{{ route('password.request') }}" class="text-xs text-emerald-600 dark:text-emerald-400 hover:underline">Forgot Password?</a>
                    </div>
        ...
        {{-- Footer Section --}}
        <footer class="grid grid-cols-2 md:grid-cols-4 gap-8 p-12 mt-4 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-black/20">
            <div class="col-span-1">
                ...
                <p class="text-[11px] text-slate-500 dark:text-gray-500 leading-relaxed">Your trusted partner in personal financial management. Safe, secure, and smart.</p>
            </div>
            <div class="text-[11px] space-y-2 text-slate-600 dark:text-gray-400">
                <p class="font-bold text-slate-900 dark:text-white mb-3 uppercase">Product</p>
            ...
            <div class="text-[11px] space-y-2 text-slate-600 dark:text-gray-400">
                <p class="font-bold text-slate-900 dark:text-white mb-3 uppercase">Support</p>
                <p class="hover:text-emerald-500 cursor-pointer transition-colors">Help Center</p>
                <p class="hover:text-emerald-500 cursor-pointer transition-colors">Security Standards</p>
            </div>
            <div class="flex flex-col items-end justify-center">
                <p class="text-[10px] text-slate-400 dark:text-gray-600 italic">© 2026 MoneyGement Inc.</p>
            </div>
        </footer>
    </div>
</body>
